...
use "$data/hcv_analytic.dta", clear
...
/*Identify first HCV claim date*/
gen dx_year=substr(first_hcv_claim_date,1,4)
destring dx_year, replace
gen dx_month=substr(first_hcv_claim_date,5,2)
destring dx_month, replace
gen dx_ym=ym(dx_year, dx_month)
gen dx_yq=qofd(dofm(dx_ym))
gen qtr_since_dx=service_yq-dx_yq

/*Identify first DAA claim*/
bysort clr_recip_id daa (service_ym): gen daa_ym=service_ym[1] if daa==1
by clr_recip_id: ereplace daa_ym=min(daa_ym)
bysort clr_recip_id daa (service_yq): gen daa_yq=service_yq[1] if daa==1
by clr_recip_id: ereplace daa_yq=min(daa_yq)
...
/*Identify advanced liver disease*/
replace adv_liver_disease=0 if adv_liver_disease==.
gsort clr_recip_id service_ym -adv_liver_disease
by clr_recip_id: replace adv_liver_disease= ///
adv_liver_disease[_n-1] if adv_liver_disease==0 & adv_liver_disease[_n-1]==1
...
save "$out/hcv_cohort_file.dta", replace
exit
