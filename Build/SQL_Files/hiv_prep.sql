...
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hiv_dx_claims;
CREATE TABLE mcaidwork.hiv_dx_claims AS
WITH cld_flags AS (
    SELECT
        cld_time_key,
        cld_claim_key
    FROM mcaid.cld
    WHERE cld_dx10_diag_code LIKE 'B20%'
       OR cld_dx10_diag_code = 'Z21'
       OR cld_dx10_diag_code LIKE 'O987%'
       OR cld_diag_code LIKE '042%'
       OR cld_diag_code = 'V08'
       OR cld_diag_code LIKE '6476%'
...
-- ============================================================
-- STEP 2: HIV member list
-- Criterion 1: >=1 inpatient claim with HIV diagnosis
-- Criterion 2: >=2 claims on different service dates
-- NOTE: Replace '01' with the inpatient claim type
--       code for this database before running
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hiv_member_list;
CREATE TABLE mcaidwork.hiv_member_list AS

-- Criterion 1: >=1 inpatient claim with HIV diagnosis
SELECT DISTINCT clr_recip_id
FROM mcaidwork.hiv_dx_claims
...
-- ============================================================
-- STEP 3: All final adjudicated claims for HIV members
-- Restricted to: exactly one record per ICN, AND
-- (clq_claim_status = '1') OR (clq_claim_status = '2' AND clq_claim_mod = '2')
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hiv_member_claims;
CREATE TABLE mcaidwork.hiv_member_claims AS
WITH adv_liver AS (
    SELECT
        cld_time_key,
        cld_claim_key,
...
base AS (
    ...
    INNER JOIN mcaidwork.hiv_member_list h
...
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hiv_enrollment_months;
CREATE TABLE mcaidwork.hiv_enrollment_months AS
...
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hiv_analytic;
CREATE TABLE mcaidwork.hiv_analytic AS
WITH first_hiv AS (
    SELECT
        clr_recip_id,
        MIN(clc_service_from_date) AS first_hiv_claim_date
    FROM mcaidwork.hiv_dx_claims
    GROUP BY clr_recip_id
),
ever_daa AS (
    SELECT DISTINCT clr_recip_id
    FROM mcaidwork.hiv_member_claims
    WHERE daa = 1
)
SELECT
...
FROM mcaidwork.hiv_enrollment_months e
LEFT JOIN mcaidwork.hiv_member_claims c
