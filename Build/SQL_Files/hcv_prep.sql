-- ============================================================
-- STEP 1: HCV claims with category flags
-- CLD is pre-aggregated to 1 row per claim (no fan-out risk)
-- before joining to CLC
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hcv_dx_claims;
CREATE TABLE mcaidwork.hcv_dx_claims AS
WITH cld_flags AS (
    SELECT
        cld_time_key,
        cld_claim_key,
        MAX(CASE WHEN cld_dx10_diag_code LIKE 'B182%'
                   OR cld_diag_code IN ('07044','07054')
             THEN 1 ELSE 0 END) AS is_chronic,
        MAX(CASE WHEN cld_dx10_diag_code IN ('B1920','B1921','Z2252')
                   OR cld_diag_code IN ('07070','07071','V0262')
             THEN 1 ELSE 0 END) AS is_unspec_carrier,
        MAX(CASE WHEN cld_dx10_diag_code LIKE 'B171%'
                   OR cld_diag_code IN ('07041','07051')
             THEN 1 ELSE 0 END) AS is_acute
    FROM mcaid.cld
    WHERE cld_dx10_diag_code LIKE 'B171%'
       OR cld_dx10_diag_code LIKE 'B182%'
       OR cld_dx10_diag_code IN ('B1920','B1921','Z2252')
       OR cld_diag_code IN ('07041','07051','07044','07054','07070','07071','V0262')
...
-- ============================================================
-- STEP 2: HCV member list — any one criterion qualifies
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hcv_member_list;
CREATE TABLE mcaidwork.hcv_member_list AS

-- Criterion 1: >=1 claim with chronic HCV
...
-- Criterion 2: >=2 claims on different dates for unspecified HCV or HCV carrier
SELECT clr_recip_id
FROM mcaidwork.hcv_dx_claims
WHERE is_unspec_carrier = 1
GROUP BY clr_recip_id
HAVING COUNT(DISTINCT clc_service_from_date) >= 2

UNION

-- Criterion 3: >=2 claims >=6 months apart for unspecified HCV, carrier, or acute HCV
SELECT DISTINCT a.clr_recip_id
FROM mcaidwork.hcv_dx_claims a
JOIN mcaidwork.hcv_dx_claims b
    ON a.clr_recip_id = b.clr_recip_id
WHERE (a.is_unspec_carrier = 1 OR a.is_acute = 1)
  AND (b.is_unspec_carrier = 1 OR b.is_acute = 1)
  AND a.clc_service_from_date < b.clc_service_from_date
  AND date_diff('month',
...
-- ============================================================
-- STEP 3: All final adjudicated claims for HCV members
-- Restricted to: exactly one record per ICN, AND
-- (clq_claim_status = '1') OR (clq_claim_status = '2' AND clq_claim_mod = '2')
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hcv_member_claims;
CREATE TABLE mcaidwork.hcv_member_claims AS
WITH adv_liver AS (
    SELECT
        cld_time_key,
        cld_claim_key,
...
base AS (
    SELECT
        ...
        CASE WHEN a.clc_procedure_code IN (
            ...
            '00074260028', -- Mavyearet
            '00074262528', -- Mavyearet
    ...
    INNER JOIN mcaidwork.hcv_member_list h
...
-- ============================================================
-- STEP 4 (revised): Enrollment year-quarters with spell number
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hcv_enrollment_months;
CREATE TABLE mcaidwork.hcv_enrollment_months AS
...
-- ============================================================
DROP TABLE IF EXISTS mcaidwork.hcv_analytic;
CREATE TABLE mcaidwork.hcv_analytic AS
WITH first_hcv AS (
    SELECT
        clr_recip_id,
        MIN(clc_service_from_date) AS first_hcv_claim_date
    FROM mcaidwork.hcv_dx_claims
    GROUP BY clr_recip_id
)
SELECT
...
FROM mcaidwork.hcv_enrollment_months e
LEFT JOIN mcaidwork.hcv_member_claims c
